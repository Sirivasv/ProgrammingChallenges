...
inline void Update(int a, int b, int v, int n, int L, int R) {
    ...
    if (L >= a && R <= b) {
		Lazy[n] = v;
		if (v == 0) for (int i = 0; i < 3; ++i) ST[n][i] = 0;
...
inline int Query(int a, int b, int n, int L, int R, int sval) {
    if (L > b || R < a) return 0;
    int mid = (L + R) >> 1;
	int tmp, lc = (n << 1), rc = (n << 1) + 1;
    if ((Lazy[n] == 1) && (L != R)) {
