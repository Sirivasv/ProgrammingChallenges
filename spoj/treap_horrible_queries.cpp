...
void Push(ptreap t) { 
  ...
  t->val += t->lazy;
  t->ans += t->lazy * t->size; //as the maximum stays just add lazy value; sum of range would involve multiply the size of the tree
...
void combine(ptreap& t, ptreap I, ptreap D) {
  ...
  t->ans = I->ans + D->ans; //combine function like SegTree
}
...
int main(){
  ...
  while (T--) {
    ...
    for (int i = 1; i <= N; ++i) Insert(myTreap, new treap(0), i);

    for (int i = 0; i < Q; ++i) {
      int op, ini, fin;
      lli val;
      cin >> op >> ini >> fin;
      if (!op) { cin >> val; RangeU(myTreap, ini, fin, val); }
      else cout << RangeQ(myTreap, ini, fin) << '\n';
    }
