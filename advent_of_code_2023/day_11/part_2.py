  ...
  for j in range(0, E_MAX_J):
    if expanded_universe[i][j] == "#":
      for galaxy_position in galaxy_positions:
        ...
        # Many empty vertical lines
        min_d_j = min(j, galaxy_position[1])
        max_d_j = max(j, galaxy_position[1])
        many_empty_vertical_lines = 0
        for e_check_j in range(min_d_j, max_d_j + 1):
          if expanded_universe[0][e_check_j] == "*":
            many_empty_vertical_lines += 1
        
        # Many empty horizontal lines
        min_d_i = min(i, galaxy_position[0])
        max_d_i = max(i, galaxy_position[0])
        many_empty_horizontal_lines = 0
        for e_check_i in range(min_d_i, max_d_i + 1):
          if expanded_universe[e_check_i][0] == "*":
            many_empty_horizontal_lines += 1

        sum_distances += abs(i - galaxy_position[0]) + abs(j - galaxy_position[1]) + (many_empty_vertical_lines*999998) + (many_empty_horizontal_lines*999998)
