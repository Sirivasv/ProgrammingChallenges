...
inline double DP(int p1, int p2, int spe1, int spe2) {
	int v = 1 + max(p1, p2);
	if (v == N - 1) return d[p1][v] + d[v][p2];
	if (memo[p1][p2][spe1][spe2] != -(1.0)) return memo[p1][p2][spe1][spe2];
	if (((v == sp) || (v == spp)) && (spe1 || spe2)) {
		if (spe1) return memo[p1][p2][spe1][spe2] = d[v][p2] + DP(p1, v, spe1, spe2 + 1);
		return memo[p1][p2][spe1][spe2] = d[p1][v] + DP(v, p2, spe1 + 1, spe2);
	}
	return memo[p1][p2][spe1][spe2] = min(d[p1][v] + DP(v, p2, spe1 + ((v == sp) || (v == spp)), spe2), d[v][p2] + DP(p1, v, spe1, spe2 + ((v == sp) || (v == spp))));
}
...
inline void reDP(int p1, int p2, int spe1, int spe2) {
	...
	double a = d[p1][v] + DP(v, p2, spe1 + ((v == sp) || (v == spp)), spe2);
	double b = d[v][p2] + DP(p1, v, spe1, spe2 + ((v == sp) || (v == spp)));
	if (b < a) {
		rightt.push(v);
